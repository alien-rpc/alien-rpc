...
function create<T extends RequestHandler = never>(
  middlewares?: T
): RouteFactory<T> {
  function route(path: string, middleware?: ExtractMiddleware<T>) {
    return defineRoute(
      path,
      middleware
        ? (middlewares?.merge(middleware) ?? chain(middleware))
        : middlewares
    )
  }
  ...
  route.use = (middleware: ExtractMiddleware<T>) => {
    return create(middlewares?.merge(middleware) ?? chain(middleware))
  }

  return route as RouteFactory<T>
}
...
export interface RouteFactory<T extends MiddlewareChain> {
   ...
   */
  <TPath extends string, TMiddleware extends ExtractMiddleware<T> = never>(
    path: TPath,
    middleware?: TMiddleware
  ): RouteBuilder<TPath, MergeMiddleware<T, TMiddleware>>
   ...
   */
  ws: <
    TArgs extends [...any[], ws.RequestContext] = [ws.RequestContext],
  ...
  use: <TMiddleware extends ExtractMiddleware<T>>(
    middleware: TMiddleware
  ) => RouteFactory<MergeMiddleware<T, TMiddleware>>
}
