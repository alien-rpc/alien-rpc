...
import {
  createTestClient as createFetch,
  CreateTestClientArgs,
} from '@hattip/adapter-test'
...
async function getTestClient() {
  ...
  return defineClient(clientRoutes.default, {
    ...
    fetch: createFetch({
      handler: chain(handler) as CreateTestClientArgs['handler'],
    }),
  })
}
